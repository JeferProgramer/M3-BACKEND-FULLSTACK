...
function problemC () {
  ...
  // promise version
  // ???
  for(let i = 1, promise = promisifiedReadFile( filenames[0]); i <= filenames.length; i++){
...
function problemE () {
  ...
  function promisifiedWriteFile (filename, str) {
    // tu código aquí
    return new Promise((resuelve, rejecta) =>{
      fs.writeFile(filename, str, (err, info) =>{
        if(err) rejecta(err)
        else resuelve(info)
      })
    })
  ...
  promisifiedWriteFile('archivo.txt', 'hola desde el atrchivo.txt')
    .then((data) =>{
      console.log(data)
    })
