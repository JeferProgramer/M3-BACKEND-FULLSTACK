...
describe("Test de APIS", () => {
  ...
  describe("POST /producto", () => {
    ...
    it("responds with 400", () => agent.post("/product").send({a:2 ,b:"jahsh"}).expect(400));
  ...
  describe("POST /numString", () => {
    ...
    it("response with status 400 if the string is empty", () => {
      agent.post("/numString").send({s: ""}).expect(400);
    })
    it("response with 4 if hola is sent", () => {
  ...
  describe('POST /pluck', () => {
    const array = [{name: 'resaltador', price:10}, {name:'lapicera', price:5}, {name:'cuadernillo', price:100}]
    it('response with status 200', () => {
      agent.post('/pluck').send({array, prop: 'name'}).expect(200)
    })
    it('response 404 if array is not an array', () => {
      agent.post('/pluck').send({array: 'martina', prop:'name'}).expect(404)
    })
    it('response with ["resaltor", "lapicera", "cuadernillo"] if called with array defined', () => {
